...
def MS_affine_cts(seed, time, scenarios, A, price, L, L_det, ypsilon, delta, a, b, C, H, pi, branching_structure, I0=None, phi_init=None, K_feat=None, lambda_fix=False): 
    ...
    if lambda_fix == False: # Cuando se fija lambda, se trabaja como parámetro -> rho y Gamma no son necesarios (incluyendo restricciones asociadas)
        ...
        #     for q in range(K_features):
        #         m.addConstr(gp.quicksum(Gamma[j, t, p, q] for p in range(pr)) == 0, name=f"sum_Gamma_{j}_{t}_{q}") 
        # # Also no need for this because lambda no longe moves in [0,1]
...
def mccormick_envelopes(model, x, y, z, x_min, x_max, y_min, y_max, name="mccormick"):
    """
    Agrega las 4 restricciones de la envolvente de McCormick al modelo Gurobi
    para linealizar la relación z = x * y.
    
    Parámetros:
    - model: El modelo de Gurobi.
    - x, y: Variables de decisión originales a multiplicar.
    - z: Nueva variable de decisión auxiliar que reemplazará a (x * y).
    - x_min, x_max: Límites (bounds) de la variable x.
    - y_min, y_max: Límites (bounds) de la variable y.
    - name: Prefijo para nombrar las restricciones.
    """
    # Restricciones de límite inferior para z
    model.addConstr(z >= x_min * y + x * y_min - x_min * y_min, name=f"{name}_lb1")
    model.addConstr(z >= x_max * y + x * y_max - x_max * y_max, name=f"{name}_lb2")
    
    # Restricciones de límite superior para z
    model.addConstr(z <= x_min * y + x * y_max - x_min * y_max, name=f"{name}_ub1")
    model.addConstr(z <= x_max * y + x * y_min - x_max * y_min, name=f"{name}_ub2")

def MS_affine_mccormick(seed, time, scenarios, A, price, L, L_det, ypsilon, delta, a, b, C, H, pi, branching_structure, I0=None, 
                                phi_init=None, K_feat=None, lambda_fix=False): 
    random.seed(seed)
    comp = len(A)
    prod = len(A[0])
    ...
    z = m.addVars(prod, time, scenarios, vtype=GRB.CONTINUOUS, name="z", lb=0) 
    ...
    f = (gp.quicksum(pi[s]*z[j,t,s] for s in range(scenarios) for j in range(prod) for t in range(time)) -
        gp.quicksum(pi[s]*H[i]*I[i,t,s] for s in range(scenarios) for i in range(comp) for t in range(time)) -
        gp.quicksum(pi[s]*C[i]*x[i,t,s] for s in range(scenarios) for i in range(comp) for t in range(time)))
    ...
    for j in range(prod):
        for t in range(time):
            for s in range(scenarios):
                # Debes definir lógicamente los min y max basándote en tu instancia
                p_min = min(price)  # Ejemplo basado en tu arreglo price
                p_max = max(price)
                d_min = 0           
                d_max = max(ypsilon[s][t] * (a - b * p_min) + delta[s][j][t] for j in range(prod) for t in range(time) for s in range(scenarios)) 
